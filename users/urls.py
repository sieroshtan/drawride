...
urlpatterns = [
    ...
    re_path(r"^user/(?P<slug>[A-Za-z0-9_]+)/drafts/$", ProfileDraftsView.as_view(), name="drafts"),
    re_path(
        r"^user/(?P<slug>[A-Za-z0-9_]+)/involved/$",
        ProfileInvolvedView.as_view(),
        name="involved",
    ),
    re_path(
        r"^user/(?P<slug>[A-Za-z0-9_]+)/favorite/$",
        ProfileFavoritesView.as_view(),
        name="favorite",
    ),
]
